export function weekNumberToDate(weekNumber, year) {
    const firstDay = new Date(year, 0, 1);
    const day = firstDay.getDay();
    const diffToMonday = day === 0 ? -6 : 1 - day;
    return new Date(year, 0, 1 + diffToMonday + (weekNumber - 1) * 7);
}
export function dateToWeekNumber(date = new Date()) {
    ...
    const weekNumber = 1 + Math.round((target.getTime() - startWeek.getTime()) / (7 * 24 * 60 * 60 * 1000));
    return weekNumber;
}
